...
public class GlitchedSpikeMediumStructure extends InfinityAndOresModElements.ModElement {
	private static Feature<NoFeatureConfig> feature = null;
	private static ConfiguredFeature<?, ?> configuredFeature = null;
	public GlitchedSpikeMediumStructure(InfinityAndOresModElements instance) {
		super(instance, 348);
	...
	private static class FeatureRegisterHandler {
		...
		public void registerFeature(RegistryEvent.Register<Feature<?>> event) {
				...
				public boolean generate(ISeedReader world, ChunkGenerator generator, Random random, BlockPos pos, NoFeatureConfig config) {
					...
					if ((random.nextInt(1000000) + 1) <= 750000) {
						...
						for (int a = 0; a < count; a++) {
							...
							if (blockAt.getBlock() == GlitchedSoilBlock.block.getDefaultState().getBlock())
								blockCriteria = true;
							if (!blockCriteria)
								continue;
							...
							Template template = world.getWorld().getStructureTemplateManager()
									.getTemplateDefaulted(new ResourceLocation("infinity_and_ores", "glitched_spike_medium"));
							if (template == null)
								return false;
			...
			event.getRegistry().register(feature.setRegistryName("glitched_spike_medium"));
			Registry.register(WorldGenRegistries.CONFIGURED_FEATURE, new ResourceLocation("infinity_and_ores:glitched_spike_medium"),
					configuredFeature);
		}
	}
	@SubscribeEvent
