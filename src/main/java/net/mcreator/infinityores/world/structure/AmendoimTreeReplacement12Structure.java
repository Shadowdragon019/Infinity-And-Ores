...
public class AmendoimTreeReplacement12Structure extends InfinityAndOresModElements.ModElement {
	private static Feature<NoFeatureConfig> feature = null;
	private static ConfiguredFeature<?, ?> configuredFeature = null;
	public AmendoimTreeReplacement12Structure(InfinityAndOresModElements instance) {
		super(instance, 422);
	...
	private static class FeatureRegisterHandler {
		...
		public void registerFeature(RegistryEvent.Register<Feature<?>> event) {
				...
				public boolean generate(ISeedReader world, ChunkGenerator generator, Random random, BlockPos pos, NoFeatureConfig config) {
					...
					if ((random.nextInt(1000000) + 1) <= 1000000) {
						...
						for (int a = 0; a < count; a++) {
							...
							BlockPos spawnTo = new BlockPos(i + -6, j + -1, k + -4);
							int x = spawnTo.getX();
							int y = spawnTo.getY();
							int z = spawnTo.getZ();
			...
			event.getRegistry().register(feature.setRegistryName("amendoim_tree_replacement_12"));
			Registry.register(WorldGenRegistries.CONFIGURED_FEATURE, new ResourceLocation("infinity_and_ores:amendoim_tree_replacement_12"),
					configuredFeature);
		}
	}
	@SubscribeEvent
