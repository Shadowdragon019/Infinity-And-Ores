...
public class MantleAmendoimForestBiome extends InfinityAndOresModElements.ModElement {
	...
	static class CustomTreeFeature extends AbstractTreeFeature<BaseTreeFeatureConfig> {
		...
		protected boolean place(IWorldGenerationReader worldgen, Random rand, BlockPos position, Set<BlockPos> changedBlocks,
			...
			if (position.getY() >= 1 && position.getY() + height + 1 <= world.getHeight()) {
				...
				if (!spawnTree) {
					...
					if (!((ground == MantleAmendoimNyliumBlock.block.getDefaultState().getBlock()
							|| ground == MantleBlock.block.getDefaultState().getBlock())
							&& (ground2 == MantleAmendoimNyliumBlock.block.getDefaultState().getBlock()
									|| ground2 == MantleBlock.block.getDefaultState().getBlock())))
						return false;
					BlockState state = world.getBlockState(position.down());
					if (position.getY() < world.getHeight() - height - 1) {
						setTreeBlockState(changedBlocks, world, position.down(), MantleBlock.block.getDefaultState(), bbox);
								...
								for (int i2 = position.getZ() - j1; i2 <= position.getZ() + j1; ++i2) {
									...
									if (Math.abs(position.getX()) != j1 || Math.abs(j2) != j1 || rand.nextInt(2) != 0 && i4 != 0) {
										...
										if (state.getBlock().isAir(state, world, blockpos) || state.getMaterial().blocksMovement()
												...
												|| state.getBlock() == AmendoimLeavesBlock.block.getDefaultState().getBlock()) {
											setTreeBlockState(changedBlocks, world, blockpos, AmendoimLeavesBlock.block.getDefaultState(), bbox);
										}
									}
								}
							}
						...
						for (int genh = 0; genh < height; genh++) {
							...
							setTreeBlockState(changedBlocks, world, genhPos, AmendoimLogBlock.block.getDefaultState(), bbox);
		...
		private boolean canGrowInto(Block blockType) {
			return blockType.getDefaultState().getMaterial() == Material.AIR || blockType == AmendoimLogBlock.block.getDefaultState().getBlock()
					|| blockType == AmendoimLeavesBlock.block.getDefaultState().getBlock()
					|| blockType == MantleAmendoimNyliumBlock.block.getDefaultState().getBlock()
					|| blockType == MantleBlock.block.getDefaultState().getBlock();
		}
