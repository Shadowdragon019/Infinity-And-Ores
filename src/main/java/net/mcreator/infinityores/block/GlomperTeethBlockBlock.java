...
public class GlomperTeethBlockBlock extends InfinityAndOresModElements.ModElement {
	...
	public static class CustomBlock extends Block {
		public CustomBlock() {
			super(Block.Properties.create(Material.PLANTS).sound(SoundType.METAL).hardnessAndResistance(-1, 3600000).lightValue(0).notSolid()
					.tickRandomly());
			setRegistryName("glomper_teeth_block");
		}
		...
		public void onEntityWalk(World world, BlockPos pos, Entity entity) {
			...
			int z = pos.getZ();
			{
				...
				$_dependencies.put("z", z);
				$_dependencies.put("world", world);
				GlompeRevealProcedureProcedure.executeProcedure($_dependencies);
			}
		}
	}
	@Override
		...
		for (Biome biome : ForgeRegistries.BIOMES.getValues()) {
			...
			biome.addFeature(GenerationStage.Decoration.UNDERGROUND_ORES, new OreFeature(OreFeatureConfig::deserialize) {
				...
				public boolean place(IWorld world, ChunkGenerator generator, Random rand, BlockPos pos, OreFeatureConfig config) {
					...
					int y = pos.getY();
					int z = pos.getZ();
					if (!GlomperTeethAdditionalGenerationProcedureProcedure.executeProcedure(ImmutableMap.of("x", x, "y", y, "z", z, "world", world)))
						return false;
					return super.place(world, generator, rand, pos, config);
				}
			}.withConfiguration(
					new OreFeatureConfig(OreFeatureConfig.FillerBlockType.create("glomper_teeth_block", "glomper_teeth_block", blockAt -> {
						boolean blockCriteria = false;
						if (blockAt.getBlock() == Blocks.CAVE_AIR.getDefaultState().getBlock())
							blockCriteria = true;
						return blockCriteria;
					}), block.getDefaultState(), 1)).withPlacement(Placement.COUNT_RANGE.configure(new CountRangeConfig(10, 0, 0, 128))));
		}
	}
}
